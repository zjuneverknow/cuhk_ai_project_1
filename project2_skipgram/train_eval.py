...
NEGATIVE_SHIFT = float(os.getenv("NEGATIVE_SHIFT", "5"))
...
def make_embeddings(cooc: sparse.csr_matrix) -> np.ndarray:
    ...
    values = np.maximum(values - math.log(NEGATIVE_SHIFT), 0.0)
    sppmi = sparse.coo_matrix((values, (coo.row, coo.col)), shape=cooc.shape).tocsr()
    k = min(DIM, min(sppmi.shape) - 2)
    u, s, vt = svds(sppmi, k=k, random_state=STUDENT_ID)
    order = np.argsort(s)[::-1]
    u, s, vt = u[:, order], s[order], vt[order, :]
    embeds = (u + vt.T) * np.sqrt(s)
...
def save_vec(vocab: list[str], embeds: np.ndarray) -> None:
    with (OUT_DIR / "skipgram.vec").open("w", encoding="utf-8") as f:
